...
contract('RedPocket', accounts => {
  ...
  it('2.B玩家发送红包给A地址，查询B信息显示推荐人为A地址', async () => {
    await sendPocket(a, { from: b, value: doubleAmount }).then(res => {
      ...
      assert.equal(res[1].toString(), a, "B玩家推荐人应该为A玩家的地址");
  ...
  it('5.A玩家4星，将A玩家升级到5星，需要总共推荐5个人，当前已推荐3个人，再邀请（含间接邀请）2人，这里省事用直推，然后发送一个红包', async () => {
    for (var i = 0; i < 2; i++) {
      var account = a_referers[i];
      await sendPocket(a, { from: account, value: doubleAmount });
    }
    console.log("升v5");
    await instance.getPlayerInfo({ from: a}).then(async res=>{
      assert.equal(4,res[2],  "A的星级应该还是4");
      console.log("发红包");
      return await sendPocket(zero, {from: a, value: doubleAmount});
    }).then(res=>{
      console.log("查状态");
      return instance.getPlayerInfo({ from: a });
    }).then(res => {
      assert.equal(5, res[2], "A的星级应该升为5");
      assert.equal(5, res[3].length, "A的直推人数应该为5人");
      assert.equal(5, res[4], "A的总人脉数应该为5人");
    }).catch(err => {
  ...
  it('last.公司地址余额变动遵循循环规律增长', async () => {
    ...
    // 总共收到的红包
    assert.equal(BN(compAmount).plus(techAmount).toFixed(), BN(16).multipliedBy(amount).toFixed(), "捡漏地址应该收到92个红包");
  });
});